...
s = t.time()
for n in range(45):
    print(makeNumberBasic(n))
print (t.time() - s)

# Intermediate difficulty
def createCombinations(z):
    from itertools import combinations
    combis = []
    digits = range(1,10)
    for digit in digits:
        combis.extend([list(c) for c in combinations(digits, digit)])
    return [sum(c) for c in combis]

def makeNumber(z):
    combinations = createCombinations(z, d)
    return len([c for c in combinations if c == z])
